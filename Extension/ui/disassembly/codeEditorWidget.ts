...
// Original file: vscode/src/vs/editor/browser/widget/codeEditorWidget.ts
...
import { Disposable, IDisposable, dispose } from './lifecycle';
...
import { CursorChangeReason, ICursorPositionChangedEvent, ICursorSelectionChangedEvent } from './controller/cursorEvents';
...
import { IIdentifiedSingleEditOperation, IModelDecoration, IModelDecorationOptions, IModelDecorationsChangeAccessor, IModelDeltaDecoration, ITextModel, ICursorStateComputer, IWordAtPosition } from './model';
import { ModelDecorationOptions } from './model/textModel';
import { IModelContentChangedEvent, IModelDecorationsChangedEvent, IModelLanguageChangedEvent, IModelLanguageConfigurationChangedEvent, IModelOptionsChangedEvent } from './model/textModelEvents';
import { VerticalRevealType } from './view/viewEvents';
import { IEditorWhitespace } from './viewLayout/linesLayout';
...
import { OutgoingViewModelEventKind } from './viewModel/viewModelEventDispatcher';
...
export class CodeEditorWidget extends Disposable implements editorBrowser.ICodeEditor {
	...
	protected _createConfiguration(options: Readonly<editorBrowser.IEditorConstructionOptions>): editorCommon.IConfiguration {
		return new Configuration(this.isSimpleWidget, options, this._domElement);
	}
	...
	public getValue(options: { preserveBOM: boolean; lineEnding: string; } | null = null): string {
		...
		return this._modelData.model.getValue(preserveBOM);
	}
			...
			switch (e.kind) {
				...
				case OutgoingViewModelEventKind.CursorStateChanged: {
					if (e.reachedMaxCursorCount) {
						console.log("The number of cursors has reached limit");
					}
